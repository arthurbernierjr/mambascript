void function () {
    var _, addValuesByInitializer, cache$, cache$1, cache$2, cache$3, checkNodes, checkType, checkTypeAnnotation, ClassScope, createIdentifier, CS, debug, extendType, fromMemberAccessToRef, fromObjectAccessToRef, FunctionScope, ImplicitAny, initializeGlobalTypes, mergeStruct, ModuleScope, reporter, resolveType, Scope, walk, walkArrayInializer, walkAssignOp, walkBinOp, walkBlock, walkBool, walkClass, walkClassProtoAssignOp, walkCompoundAssignOp, walkConditional, walkDynamicMemberAccessOp, walkDynamicProtoMemberAccessOp, walkFloat, walkFor, walkFunction, walkFunctionApplication, walkIdentifier, walkInt, walkMemberAccess, walkModule, walkNewOp, walkNull, walkNumbers, walkObjectInitializer, walkOfOp, walkPrimitives, walkProgram, walkProtoMemberAccessOp, walkRange, walkReturn, walkString, walkStruct, walkSwitch, walkThis, walkUndefined, walkVardef;
    ...
    fromObjectAccessToRef = function (node) {
        var cur, h, head, parent;
        head = {
            left: {},
    ...
    walkVardef = function (node, scope) {
        ...
        if (scope instanceof ClassScope || scope instanceof ModuleScope) {
            ...
            debug('vardef getVar', scope);
            if (!(val = scope.getVar(symbol))) {
                return scope.addVar({
                    nodeType: 'variable',
                    identifier: {
                        typeRef: symbol,
    ...
    walkIdentifier = function (node, scope) {
        var mod, typeAnnotation, typeName, val;
        typeName = node.data;
        if (val = scope.getVarInScope(typeName)) {
            typeAnnotation = null != val ? val.typeAnnotation : void 0;
            node.typeAnnotation = null != typeAnnotation ? typeAnnotation : ImplicitAny;
    ...
    walkClass = function (node, scope) {
        var arg, className, classScope, ident, implArg, ns, parentAnnotation, parentStaticAnnotation, prop, statement, staticAnn;
