void function () {
    var _, addValuesByInitializer, cache$, cache$1, checkNodes, checkType, checkTypeAnnotation, ClassScope, compareAsParent, CS, debug, extendFunctionType, extendTypeWithArguments, FunctionScope, ImplicitAnyAnnotation, initializeGlobalTypes, isAcceptable, reporter, resolveType, Scope, walk, walkArrayInializer, walkAssignOp, walkBinOp, walkBlock, walkBool, walkClass, walkClassProtoAssignOp, walkCompoundAssignOp, walkConditional, walkDynamicMemberAccessOp, walkDynamicProtoMemberAccessOp, walkFloat, walkFor, walkFunction, walkFunctionApplication, walkIdentifier, walkInt, walkMemberAccess, walkNewOp, walkNull, walkNumbers, walkObjectInitializer, walkOfOp, walkPrimitives, walkProgram, walkProtoMemberAccessOp, walkRange, walkReturn, walkString, walkStruct, walkSwitch, walkThis, walkUndefined, walkVardef;
    ...
    extendTypeWithArguments = cache$.extendTypeWithArguments;
    extendFunctionType = cache$.extendFunctionType;
    ...
    walkBlock = function (node, scope) {
        var ann, cache$2, cache$3, head, last_typeAnnotation, returnables, tail;
        ...
        if (null != (cache$2 = _.last(node.statements)) ? cache$2.typeAnnotation : void 0)
            returnables.push(_.last(node.statements).typeAnnotation);
        cache$3 = returnables;
        head = cache$3[0];
        tail = 2 <= cache$3.length ? [].slice.call(cache$3, 1) : [];
    ...
    walkNewOp = function (node, scope) {
        var arg, ctorAnnotation, givenArgs, left, n, right, type;
        type = scope.getTypeInScope(node.ctor.data);
        if (null != node.ctor.typeArguments ? node.ctor.typeArguments.length : void 0) {
    ...
    walkFunctionApplication = function (node, scope) {
        var arg, givenArg, left, n, right, type, typeArguments, typeScope;
        for (var i$ = 0, length$ = node['arguments'].length; i$ < length$; ++i$) {
            arg = node['arguments'][i$];
            walk(arg, scope);
        }
    ...
    walk = function (node, scope) {
        ...
        if (!(null != node))
            return;
        console.error('walking node:', null != node ? node.className : void 0, null != node ? node.raw : void 0);
        switch (false) {
        case !!(null != node):
            return;
