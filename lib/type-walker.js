void function () {
    ...
    cache$2 = require('./types');
    initializeGlobalTypes = cache$2.initializeGlobalTypes;
    Scope = cache$2.Scope;
    ...
    walkBlock = function (node, scope) {
        var cache$3, lastAnn, returnables;
        walk(node.statements, scope);
        returnables = scope.getReturnables();
        if (null != (cache$3 = _.last(node.statements)) ? cache$3.typeAnnotation : void 0) {
    ...
    walkBinOp = function (node, scope) {
        var cache$3, leftAnnotation, leftRef, rightAnnotation, rightRef;
        ...
        leftAnnotation = cache$3[0];
        rightAnnotation = cache$3[1];
    ...
    walkSwitch = function (node, scope) {
        ...
        if (null != ann) {
            ...
            return node.typeAnnotation = null != ann ? ann : ImplicitAnyAnnotation;
        } else {
            return node.typeAnnotation = ImplicitAnyAnnotation;
        }
    };
    ...
    walkClassProtoAssignOp = function (node, scope) {
        var annotation, cache$3, left, right, symbol;
        left = node.assignee;
        right = node.expression;
        symbol = left.data;
        if (right['instanceof'](CS.Function) && scope.getThis(symbol)) {
            annotation = null != (cache$3 = scope.getThis(symbol)) ? cache$3.typeAnnotation : void 0;
            if (null != annotation) {
    ...
    walkIdentifier = function (node, scope) {
        var cache$3, typeAnnotation, typeName;
        typeName = node.data;
        if (scope.getVarInScope(typeName)) {
            typeAnnotation = null != (cache$3 = scope.getVarInScope(typeName)) ? cache$3.typeAnnotation : void 0;
    ...
    walkObjectInitializer = function (node, scope) {
        var cache$3, expression, key, nextScope, obj, props;
        obj = {};
        ...
        for (var i$ = 0, length$ = node.members.length; i$ < length$; ++i$) {
            {
                cache$3 = node.members[i$];
                expression = cache$3.expression;
                key = cache$3.key;
            }
    ...
    walkClass = function (node, scope) {
        var arg, classScope, impl, parentAnnotation, statement, type;
