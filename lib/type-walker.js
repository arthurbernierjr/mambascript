...
walkClassProtoAssignOp = function (node, scope) {
  ...
  symbol = left.data;
  if (right['instanceof'](MS.Function) && scope.getThis(symbol)) {
...
walkAssignOp = function (node, scope) {
  ...
  walk(left, scope);
  if (right['instanceof'](MS.Function) && scope.getVarInScope(symbol)) {
    v = scope.getVarInScope(symbol);
    walkFunction(right, scope, v.typeAnnotation);
  } else if (right['instanceof'](MS.Function) && preAnnotation) {
  ...
  if (left instanceof MS.ArrayInitialiser) {
    if ((null != right.typeAnnotation && null != right.typeAnnotation.identifier ? right.typeAnnotation.identifier.typeRef : void 0) === 'Any') {
      '';
    } else if (right instanceof MS.ArrayInitialiser) {
...
walkPrimitives = function (node, scope) {
  switch (false) {
  case !node['instanceof'](MS.String):
    return walkString(node, scope);
  case !node['instanceof'](MS.Bool):
    return walkBool(node, scope);
  case !node['instanceof'](MS.Int):
    return walkInt(node, scope);
  case !node['instanceof'](MS.Float):
    return walkFloat(node, scope);
  case !node['instanceof'](MS.Numbers):
    return walkNumbers(node, scope);
  case !node['instanceof'](MS.Null):
    return walkNull(node, scope);
  case !node['instanceof'](MS.Undefined):
...
walkClass = function (node, scope) {
  ...
  className = null;
  isThis = false;
  if ((null != node ? node.nameAssignee : void 0) instanceof MS.Identifier) {
  ...
  } else if ((null != node ? node.nameAssignee : void 0) instanceof MS.MemberAccessOp) {
...
walk = function (node, scope) {
  ...
  case !node['instanceof'](MS.Program):
    return walkProgram(node, scope);
  case !node['instanceof'](MS.Module):
    return walkModule(node, scope);
  case !node['instanceof'](MS.Block):
    return walkBlock(node, scope);
  case !node['instanceof'](MS.Return):
    return walkReturn(node, scope);
  case !node['instanceof'](MS.NewOp):
    return walkNewOp(node, scope);
  case !(node['instanceof'](MS.PlusOp) || node['instanceof'](MS.MultiplyOp) || node['instanceof'](MS.DivideOp) || node['instanceof'](MS.SubtractOp)):
    return walkBinOp(node, scope);
  case !node['instanceof'](MS.Switch):
    return walkSwitch(node, scope);
  case !node['instanceof'](MS.Conditional):
    return walkConditional(node, scope);
  case !node['instanceof'](MS.Undefined):
    return walkUndefined(node, scope);
  case !node['instanceof'](MS.OfOp):
    return walkOfOp(node, scope);
  case !(node['instanceof'](MS.ForIn) || node['instanceof'](MS.ForOf)):
    return walkFor(node, scope);
  case !node['instanceof'](MS.Primitives):
    return walkPrimitives(node, scope);
  case !node['instanceof'](MS.Null):
    return walkNull(node, scope);
  case !node['instanceof'](MS.This):
    return walkThis(node, scope);
  case !node['instanceof'](MS.Identifier):
    return walkIdentifier(node, scope);
  case !node['instanceof'](MS.ClassProtoAssignOp):
    return walkClassProtoAssignOp(node, scope);
  case !node['instanceof'](MS.DynamicProtoMemberAccessOp):
    return walkDynamicProtoMemberAccessOp(node, scope);
  case !node['instanceof'](MS.DynamicMemberAccessOp):
    return walkDynamicMemberAccessOp(node, scope);
  case !node['instanceof'](MS.ProtoMemberAccessOp):
    return walkProtoMemberAccessOp(node, scope);
  case !node['instanceof'](MS.MemberAccessOps):
    return walkMemberAccess(node, scope);
  case !node['instanceof'](MS.ArrayInitialiser):
    return walkArrayInializer(node, scope);
  case !node['instanceof'](MS.Range):
    return walkRange(node, scope);
  case !node['instanceof'](MS.ObjectInitialiser):
    return walkObjectInitializer(node, scope);
  case !node['instanceof'](MS.Class):
    return walkClass(node, scope);
  case !(node['instanceof'](MS.Function) || node['instanceof'](MS.BoundFunction)):
    return walkFunction(node, scope);
  case !node['instanceof'](MS.FunctionApplication):
    return walkFunctionApplication(node, scope);
  case !node['instanceof'](MS.CompoundAssignOp):
    return walkCompoundAssignOp(node, scope);
  case !node['instanceof'](MS.AssignOp):
