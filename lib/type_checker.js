...
walk_for = function (node, scope) {
  ...
  if (null != (null != node.target.annotation && null != node.target.annotation.type ? node.target.annotation.type.array : void 0)) {
    for (var i$ = 0, length$ = (null != node.target.annotation && null != node.target.annotation.type ? node.target.annotation.type.array : void 0).length; i$ < length$; ++i$) {
      el = (null != node.target.annotation && null != node.target.annotation.type ? node.target.annotation.type.array : void 0)[i$];
      if (null != node.valAssignee)
...
walk_assignOp = function (node, scope) {
  var assigning, el, infered, left, registered, right, symbol;
  left = node.assignee;
