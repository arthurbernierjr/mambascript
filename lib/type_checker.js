...
walk_assignOp = function (node, scope) {
  var err, l_type, left, member, pre_registered_annotation, right, symbol, T;
  ...
  if (null != (null != left ? left.members : void 0)) {
    for (var i$ = 0, length$ = left.members.length; i$ < length$; ++i$) {
      ...
      if (scope.getVarInScope(member.key.data)) {
        ...
        if (err = scope.checkAcceptableObject(l_type, null != right.annotation && null != right.annotation.type ? right.annotation.type[member.key.data] : void 0))
          return reporter.add_error(node, err);
      } else {
