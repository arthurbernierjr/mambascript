...
// Generated by KofuScript 0.0.4-alpha.5 
var addHistory, addMultilineHandler, CS, fs, KofuScript, merge, nodeREPL, path, reporter, vm;
fs = require('fs');
path = require('path');
vm = require('vm');
...
module.exports = {
  start: function (opts) {
    ...
    if (null == opts)
      opts = {};
    opts.prompt || (opts.prompt = 'kofuScript\ud83d\udc10>> ');
    ...
    opts['eval'] || (opts['eval'] = function (input, context, filename, cb) {
      ...
      if (/^\s*$/.test(input))
        return cb(null);
      try {
        inputAst = KofuScript.parse(input, {
          filename: filename,
          raw: true
        });
        transformedAst = new CS.AssignOp(new CS.Identifier('_'), inputAst.body);
        jsAst = KofuScript.compile(transformedAst, {
          bare: true,
          inScope: Object.keys(context)
        });
        js = KofuScript.js(jsAst);
