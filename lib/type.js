...
_typecheck = function (node, parentScope) {
  var assigned_type, assignee, body, cache$, cache$1, expression, fnode, infered_type, name, registered_type, snode, symbol;
  if (node === void 0)
    return;
  if (null != node.length) {
    ...
    if (null != registered_type && null != assigned_type) {
      ...
      if (infered_type === 'Function' && null != (null != expression.body ? expression.body.statements : void 0)) {
        fnode = new ScopeNode;
        fnode.name = symbol;
        fnode.parent = parentScope;
        parentScope.nodes.push(fnode);
