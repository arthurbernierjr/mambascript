...
var checkNodes, CS, initializeGlobalTypes, Scope, TypeSymbol, VarSymbol, walk;
CS = require('./nodes');
VarSymbol = function () {
...
walk = function (node, currentScope) {
  var assigning, cache$, expected, expression, infered, key, left, nextScope, obj, objectScope, registered, right, symbol, val;
  switch (false) {
  case !(node === void 0):
    return;
  ...
  case !node['instanceof'](CS.Function):
    ...
    node.parameters.map(function (param) {
      try {
        if ('function' === typeof objectScope.setVar)
          return objectScope.setVar(param.data, null != (null != param.annotation ? param.annotation.type : void 0) ? null != param.annotation ? param.annotation.type : void 0 : 'Any');
      } catch (e$) {
    ...
    if (null != assigning && null != registered) {
      ...
      if (assigning === 'Any') {
        ...
        for (key in assigning) {
          val = assigning[key];
          if (right.annotation.type[key] !== val)
            throw new Error("'" + key + "' is expected to " + right.annotation.type[key] + '(indeed ' + val + ')');
        }
        return currentScope.setVar(symbol, left.annotation.type, false);
