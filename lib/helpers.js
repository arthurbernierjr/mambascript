void function () {
    ...
    this.pointToErrorLocation = pointToErrorLocation = function (source, line, column, numLinesOfContext) {
        ...
        if (startLine < 0)
            startLine = 0;
        preLines = lines.slice(startLine, +currentLineOffset + 1 || 9000000000);
        preLines[preLines.length - 1] = colourise('yellow', preLines[preLines.length - 1]);
        postLines = lines.slice(currentLineOffset + 1, +(currentLineOffset + numLinesOfContext) + 1 || 9000000000);
