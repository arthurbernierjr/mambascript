...
// Generated by MambaScript 0.5.2 
var $0, $0, additionalArgs, concat, escodegen, esmangle, foldl, fs, humanReadable, input, input, inputName, inputSource, inspect, knownOpts, MambaScript, mscodegen, nopt, numberLines, optAliases, Optimiser, option, options, output, parameter, path, pkg, positionalArgs, Preprocessor, processInput, Repl, runMain;
fs = require('fs');
path = require('path');
...
if (!(options.compile || options.js || options.sourceMap || options.parse || options['eval'] || options.mscodegen))
  if (!(null != escodegen)) {
...
if (1 !== (null != options.parse ? options.parse : 0) + (null != options.compile ? options.compile : 0) + (null != options.js ? options.js : 0) + (null != options.sourceMap ? options.sourceMap : 0) + (null != options['eval'] ? options['eval'] : 0) + (null != options.mscodegen ? options.mscodegen : 0) + (null != options.repl ? options.repl : 0)) {
...
if (options.mscodegen && !(null != mscodegen)) {
...
if (options.help) {
  ...
  processInput = function (err) {
    ...
    if (options.mscodegen) {
      try {
        result = mscodegen.generate(result);
      } catch (e$5) {
        e = e$5;
    ...
    if (options.sourceMap) {
      try {
        sourceMap = MambaScript.sourceMap(jsAST, inputName, { compact: options.minify });
      } catch (e$7) {
        e = e$7;
        console.error(e.stack || e.message);
        process.exit(1);
    ...
    try {
      cache$3 = MambaScript.jsWithSourceMap(jsAST, inputName, { compact: options.minify });
      js = cache$3.code;
