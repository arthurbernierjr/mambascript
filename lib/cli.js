...
// Generated by KofuScript 0.0.4-alpha.5 
var $0, $0, additionalArgs, concat, cscodegen, escodegen, esmangle, foldl, fs, humanReadable, input, input, inputName, inputSource, inspect, knownOpts, KofuScript, nopt, numberLines, optAliases, Optimiser, option, options, output, parameter, path, pkg, positionalArgs, Preprocessor, processInput, Repl, runMain;
fs = require('fs');
path = require('path');
...
if (options.help) {
  ...
  processInput = function (err) {
    ...
    jsAST = KofuScript.compile(result, { bare: options.bare });
    if (options.compile)
      if (null != jsAST) {
    ...
    if (options.sourceMap) {
      try {
        sourceMap = KofuScript.sourceMap(jsAST, inputName, { compact: options.minify });
      } catch (e$7) {
        e = e$7;
        console.error(e.stack || e.message);
        process.exit(1);
    ...
    try {
      cache$3 = KofuScript.jsWithSourceMap(jsAST, inputName, { compact: options.minify });
      js = cache$3.code;
