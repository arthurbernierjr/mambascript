...
// Generated by MambaScript 0.1.4 
var DEDENT, INDENT, pointToErrorLocation, Preprocessor, reminder, StringScanner, TERM, ws;
pointToErrorLocation = require('./helpers').pointToErrorLocation;
StringScanner = require('StringScanner');
reminder = 'no big deal take a look at your code. Are you sure you are following the KOFU principles of writing Unfancy Typed JavaScript';
    ...
    if (this.ss.bol() || this.scan(new RegExp('(?:[' + ws + ']*\\n)+'))) {
      ...
      if (null != this.base) {
        if (!(this.ss.eos() || null != this.scan(this.base))) {
          throw new Error('inconsistent base indentation remember white space matters... Think Harvard Outline');
        }
      } else {
      ...
      while (indentIndex < this.indents.length) {
        ...
        if (this.ss.check(new RegExp('' + indent + ''))) {
          ...
          lines = this.ss.str.substr(0, this.ss.pos).split(/\n/) || [''];
          message = 'Syntax error on line ' + lines.length + ': indentation is ambiguous make sure you use the same indentation level on each line';
  ...
  Preprocessor.prototype.process = function (input) {
    ...
    while (this.context.length) {
      ...
      case '#':
        ...
        break;
      default:
        throw new Error('Unclosed "' + this.peek().replace(/"/g, '\\"') + '" at EOF. ' + reminder);
      }
    }
